...
export const runtimeModelProviderSchema = z.enum(["cline", "ollama", "openai-compatible", "custom"]);
export type RuntimeModelProvider = z.infer<typeof runtimeModelProviderSchema>;

export const runtimeModelProfileSchema = z.object({
	id: z.string(),
	label: z.string(),
	...
	supportedRoles: z.array(runtimeAgentRoleSchema),
	createdAt: z.number(),
	updatedAt: z.number(),
});
...
export const runtimeProjectRuleSchema = z.object({
	...
	contextExcludeGlobs: z.array(z.string()).default([]),
	preferredTestCommands: z.array(z.string()).default([]),
	updatedAt: z.number(),
});
export type RuntimeProjectRule = z.infer<typeof runtimeProjectRuleSchema>;

export const runtimeAutopilotPolicySchema = z.object({
	enabled: z.boolean(),
	allowedActions: z.array(z.enum(["assign_task", "run_worker", "validate_patch", "apply_patch", "run_tests"])),
	maxLoopCount: z.number().int().nonnegative(),
	requireApprovalForPatchApply: z.boolean(),
	requireApprovalForTestRun: z.boolean(),
});
export type RuntimeAutopilotPolicy = z.infer<typeof runtimeAutopilotPolicySchema>;

export const runtimeWorkerStatusSchema = z.enum(["online", "offline", "busy", "error"]);
export type RuntimeWorkerStatus = z.infer<typeof runtimeWorkerStatusSchema>;

export const runtimeWorkerKindSchema = z.enum(["local", "remote"]);
export type RuntimeWorkerKind = z.infer<typeof runtimeWorkerKindSchema>;

export const runtimeWorkerModelInfoSchema = z.object({
	name: z.string(),
	size: z.number().int().nonnegative().nullable().optional(),
	modifiedAt: z.string().nullable().optional(),
});
export type RuntimeWorkerModelInfo = z.infer<typeof runtimeWorkerModelInfoSchema>;

export const runtimeWorkerNodeSchema = z.object({
	id: z.string(),
	name: z.string(),
	kind: runtimeWorkerKindSchema,
	status: runtimeWorkerStatusSchema,
	ollamaUrl: z.string().nullable(),
	models: z.array(runtimeWorkerModelInfoSchema),
	currentTaskId: z.string().nullable(),
	lastHeartbeatAt: z.number().nullable(),
	metrics: z
		.object({
			activeRuns: z.number().int().nonnegative().optional(),
			completedRuns: z.number().int().nonnegative().optional(),
			lastRunDurationMs: z.number().int().nonnegative().nullable().optional(),
		})
		.optional(),
...
export const runtimePromptRunSchema = z.object({
	id: z.string(),
	workspaceId: z.string(),
	taskId: z.string().nullable().optional(),
	role: runtimeAgentRoleSchema,
	modelProfileId: z.string().nullable(),
	contextBundleId: z.string().nullable().optional(),
	prompt: z.string(),
	response: z.string().nullable(),
	status: runtimePromptRunStatusSchema,
	error: z.string().nullable().optional(),
	startedAt: z.number().nullable(),
	completedAt: z.number().nullable(),
	createdAt: z.number(),
});
export type RuntimePromptRun = z.infer<typeof runtimePromptRunSchema>;

export const runtimeProjectChatMessageSchema = z.object({
	id: z.string(),
	conversationId: z.string(),
	role: z.enum(["user", "assistant", "system", "status"]),
	mode: z.enum(["plan", "architect"]),
	content: z.string(),
	promptRunId: z.string().nullable().optional(),
	createdAt: z.number(),
});
export type RuntimeProjectChatMessage = z.infer<typeof runtimeProjectChatMessageSchema>;

export const runtimeProjectChatConversationSchema = z.object({
	id: z.string(),
	workspaceId: z.string(),
	mode: z.enum(["plan", "architect"]),
	title: z.string().nullable(),
	modelProfileId: z.string().nullable(),
	messages: z.array(runtimeProjectChatMessageSchema),
	createdAt: z.number(),
	updatedAt: z.number(),
});
export type RuntimeProjectChatConversation = z.infer<typeof runtimeProjectChatConversationSchema>;

export const runtimeArchitectTaskDraftSchema = z.object({
	id: z.string(),
	title: z.string(),
	prompt: z.string(),
	acceptanceCriteria: z.array(z.string()).default([]),
	suggestedFiles: z.array(z.string()).default([]),
	dependencyTaskDraftIds: z.array(z.string()).default([]),
	baseRef: z.string().nullable().optional(),
	priority: z.enum(["low", "medium", "high"]).optional(),
	estimatedDifficulty: z.enum(["low", "medium", "high"]).optional(),
});
export type RuntimeArchitectTaskDraft = z.infer<typeof runtimeArchitectTaskDraftSchema>;

export const runtimePatchProposalStatusSchema = z.enum([
	"draft",
	"submitted",
	"validated",
	"rejected",
...
export type RuntimePatchProposalStatus = z.infer<typeof runtimePatchProposalStatusSchema>;

export const runtimePatchProposalFileSchema = z.object({
	path: z.string(),
	status: runtimeWorkspaceFileStatusSchema,
	additions: z.number().int().nonnegative().optional(),
	deletions: z.number().int().nonnegative().optional(),
});
export type RuntimePatchProposalFile = z.infer<typeof runtimePatchProposalFileSchema>;

export const runtimePatchProposalSchema = z.object({
	id: z.string(),
	taskId: z.string(),
	...
	patchText: z.string(),
	summary: z.string(),
	files: z.array(runtimePatchProposalFileSchema),
	status: runtimePatchProposalStatusSchema,
	error: z.string().nullable().optional(),
	createdAt: z.number(),
	updatedAt: z.number(),
});
export type RuntimePatchProposal = z.infer<typeof runtimePatchProposalSchema>;

export const runtimeWorkerTaskResultStatusSchema = z.enum(["completed", "failed", "cancelled"]);
export type RuntimeWorkerTaskResultStatus = z.infer<typeof runtimeWorkerTaskResultStatusSchema>;

export const runtimeWorkerTaskResultSchema = z.object({
	taskId: z.string(),
	workerNodeId: z.string(),
	model: z.string(),
	status: runtimeWorkerTaskResultStatusSchema,
	summary: z.string().nullable(),
	explanation: z.string().nullable(),
	patchText: z.string().nullable(),
