...
describe.sequential("runtime-config auto agent selection", () => {
	...
	it("defaults and persists the Ollama base URL", async () => {
		...
		try {
			await withTemporaryEnv({ home: tempHome }, async () => {
				const current = await loadRuntimeConfig(tempProject);
				expect(current.ollamaBaseUrl).toBe("http://127.0.0.1:11434");

				const updated = await updateRuntimeConfig(tempProject, {
					ollamaBaseUrl: " http://localhost:11435 ",
				});
				expect(updated.ollamaBaseUrl).toBe("http://localhost:11435");
				...
				expect(globalPayload.ollamaBaseUrl).toBe("http://localhost:11435");

				const reloaded = await loadRuntimeConfig(tempProject);
				expect(reloaded.ollamaBaseUrl).toBe("http://localhost:11435");
			});
		} finally {
			cleanupProject();
			cleanupHome();
